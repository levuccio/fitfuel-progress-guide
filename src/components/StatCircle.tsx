...
interface StatCircleProps {
  ...
  imageSrc?: string;   // Optional pixel avatar image path
}
...
export function StatCircle({
  ...
  return (
    <div className="flex flex-col items-center text-center p-2">
      <div
        className={cn(
          "relative w-18 h-18 rounded-full flex items-center justify-center mb-1",
          colorClass
        )}
      >
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={label}
            className="absolute inset-0 m-auto h-12 w-12 rounded-full pixelated object-contain"
          />
        ) : (
